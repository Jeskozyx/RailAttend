    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Data Jabatan</h1>
                    <p class="mt-1 text-sm text-gray-500">Kelola data jabatan</p>
                </div>
                <a href="{{ route('role.create') }}" class="px-6 py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-xl hover:from-blue-700 hover:to-blue-800 transition-all duration-200 shadow-lg hover:shadow-xl flex items-center space-x-2">
                        ...
                        @forelse ($role as $item)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('role.edit', ['id' => $item->id]) }}" class="text-blue-600 hover:text-blue-900 transition-colors p-2 hover:bg-blue-50 rounded-lg" title="Edit">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white px-6 py-4 border-t border-gray-200">
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div class="text-sm text-gray-700">
                            Menampilkan 
                            <span class="font-semibold">{{ $role->firstItem() ?? 0 }}</span> 
                            sampai 
                            <span class="font-semibold">{{ $role->lastItem() ?? 0 }}</span> 
                            dari 
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            @else
                                <a href="{{ $role->appends(request()->query())->previousPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-all duration-200">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                                    </svg>
                                </a>
                            ...
                            {{-- Page Numbers --}}
                            @foreach ($role->getUrlRange(1, $role->lastPage()) as $page => $url)
                                @if ($page == $role->currentPage())
                                    <button class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium transition-all duration-200">
                                        {{ $page }}
                                    </button>
                                @else
                                    <a href="{{ $role->appends(request()->query())->url($page) }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-all duration-200">
                                        {{ $page }}
                                    </a>
                                @endif
                            @endforeach
                            ...
                            @if ($role->hasMorePages())
                                <a href="{{ $role->appends(request()->query())->nextPageUrl() }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 transition-all duration-200">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
